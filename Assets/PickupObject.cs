...
public class PickupObject : MonoBehaviour
{
    ...
    private bool isPickedUp = false; // New flag to track whether the object has been picked up
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (!isPickedUp) // Check if the object has already been picked up
            {
                rb.isKinematic = true;
                coll.enabled = false;
                canTrigger = false;
                target = other.transform;
                isPickedUp = true; // Set the flag to true indicating the object has been picked up
            }
        }
    }
    ...
    void DropObject()
    {
        if (!canTrigger && target != null)
        {
            ...
            canTrigger = true;
            target = null;
            isPickedUp = false; // Reset the flag indicating the object has been dropped
